...
template <typename KeyType, typename ValueType,
    typename KeyCompareFunc>
bool skiplist<KeyType, ValueType, KeyCompareFunc>::
    compare_node(node_type* node, const KeyType& key) const {
    return node != nullptr && _key_compare_func(node->key, key);
}

template <typename KeyType, typename ValueType,
    typename KeyCompareFunc>
void skiplist<KeyType, ValueType, KeyCompareFunc>::
clear() {
...
template <typename KeyType, typename ValueType,
    typename KeyCompareFunc>
unsigned skiplist<KeyType, ValueType, KeyCompareFunc>::
size() const {
    return _size; 
}
...
template <typename KeyType, typename ValueType,
    typename KeyCompareFunc>
bool skiplist<KeyType, ValueType, KeyCompareFunc>::
search(const KeyType& key, ValueType* value) const {
    node_type* cur = _head;
...
template <typename KeyType, typename ValueType,
    typename KeyCompareFunc>
bool skiplist<KeyType, ValueType, KeyCompareFunc>::
erase(const KeyType& key, ValueType *value) {
    node_type* update_nodes[skiplist_space::MAX_LEVEL] = {0};
    node_type* cur = _head;
    for (int i = skiplist_space::MAX_LEVEL - 1; i >= 0; --i) {
        while (compare_node(cur->forward[i], key)) {
...
template <typename KeyType, typename ValueType,
    typename KeyCompareFunc>
void skiplist<KeyType, ValueType, KeyCompareFunc>::
for_each(std::function<void(KeyType*, ValueType*)> func, bool reverse) const {
    if (reverse) {
